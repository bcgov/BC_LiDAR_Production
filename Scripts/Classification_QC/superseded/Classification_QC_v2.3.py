...
class ClassificationQC:
    def __init__(self, root):
        self.root = root
        self.root.title("Classification_QC_v2.3")
        self.root.geometry("600x425")

        # Get the path to the bundled icon
        if getattr(sys, 'frozen', False):
            # Running as PyInstaller bundle
            icon_path = os.path.join(sys._MEIPASS, "Classification_QC.ico")
        else:
            # Running as script
            icon_path = r"Z:\SPENCER_FLOYD\.ico\Classification_QC.ico"
    ...
    def merge_geotiffs_by_island(self, input_dir, output_base, nodata_value=0):
        ...
        for idx, island in enumerate(islands, start=1):
            ...
            out_meta.update({
                ...
                "transform": out_trans,
                "nodata": nodata_value,
                "crs": f"EPSG:{epsg_code}"  # Explicitly set CRS
            })
