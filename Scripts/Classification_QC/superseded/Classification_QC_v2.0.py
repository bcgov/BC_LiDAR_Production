...
class ClassificationQC:
    def __init__(self, root):
        self.root = root
        self.root.title("Classification_QC_v2.0")
    ...
    def erode_valid_data(src, nodata_value, erosion_pixels=25):
        ...
        # Erode the mask to shrink valid data inward
        eroded_mask = binary_erosion(valid_mask, iterations=erosion_pixels)

        # Apply eroded mask: Set newly eroded pixels to nodata
    ...
    def start_processing(self):
        ...
        if self.high_noise_range_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -nodata 0 -no_world_file -no_kml -cpu64 -range -cores {self.cores_var.get()} -false -quiet -keep_class 1 3 4 5 -otif -odir "{high_noise_range_dir}" -step 8')
        ...
        if self.class_7_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -cpu64 -cores {self.cores_var.get()} -step 2 -false -quiet -no_world_file -no_kml -nodata 0 -quiet -keep_class 7 -point_density -otif -odir "{class_7_dir}"')
            self.print_rainbow_dashes()
        ...
        if self.ground_density_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -no_world_file -no_kml -cpu64 -cores {self.cores_var.get()} -point_density -false -quiet -keep_class 2 -otif -odir "{ground_density_dir}" -step 2')
            self.print_rainbow_dashes()
        ...
        if self.default_density_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -no_world_file -no_kml -cpu64 -cores {self.cores_var.get()} -point_density -false -quiet -keep_class 1 -otif -odir "{default_density_dir}" -step 2')
            self.print_rainbow_dashes()
        ...
        if self.geowizard_density_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -no_world_file -no_kml -cpu64 -cores {self.cores_var.get()} -point_density -false -quiet -keep_class 18 -otif -odir "{geowizard_density_dir}" -step 5')
            self.print_rainbow_dashes()
        ...
        if self.hillshade_var.get():
            ...
            os.system(f'blast2dem -i "{self.input_dir_path}\\*.las" -cores {self.cores_var.get()} -hillshade -no_kml -no_world_file -nodata 0 -keep_class 2 -otif -kill 100 -odir "{hillshade_dir}" -step 1')
            self.print_rainbow_dashes()
