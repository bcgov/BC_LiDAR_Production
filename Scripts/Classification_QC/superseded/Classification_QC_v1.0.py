...
class ClassificationQC:
    def __init__(self, root):
        self.root = root
        self.root.title("Classification_QC_v1.0")
    ...
    def start_processing(self):
        if not self.hillshade_var.get() and not self.high_noise_var.get() and not self.class_7_var.get() and not self.DSM_var.get() and not self.low_noise_var.get():
        ...
        if self.DSM_var.get():
            ...
            os.system(f'blast2dem -i "{self.input_dir_path}\\*.las" -merged -o DSM.tif -no_kml -no_world_file -nodata 0 -fail -first_only -kill 500 -odir "{DSM_dir}" -step {self.spatial_resolution_var.get()}')
            self.print_rainbow_dashes()
