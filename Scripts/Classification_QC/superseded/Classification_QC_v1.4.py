...
class ClassificationQC:
    def __init__(self, root):
        self.root = root
        self.root.title("Classification_QC_v1.4")
        ...
        self.high_noise_rasters_label = tk.Label(root, text="High Noise Rasters")
        self.high_noise_rasters_label.pack(pady=10)

        self.high_noise_var = tk.IntVar(value=0)
        ...
        self.low_noise_rasters_label = tk.Label(root, text="Low Noise Rasters")
        self.low_noise_rasters_label.pack(pady=10)

        self.low_noise_var = tk.IntVar(value=0)
        ...
        self.low_noise_range_var = tk.IntVar(value=0)
        ...
        self.outlier_rasters_label = tk.Label(root, text="Outlier Raster")
        self.outlier_rasters_label.pack(pady=10)
    ...
    def start_processing(self):
        ...
        if not self.hillshade_var.get() and not self.high_noise_var.get() and not self.high_noise_range_var.get() and not self.default_density_var.get() and not self.ground_density_var.get() and not self.class_7_var.get() and not self.low_noise_var.get() and not self.low_noise_range_var.get():
        ...
        if self.high_noise_range_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -nodata 0 -no_world_file -no_kml -cpu64 -range -cores {self.cores_var.get()} -false -fail -fill 1000 -quiet -keep_class 1 3 4 5 -otif -odir "{high_noise_range_dir}" -step {self.spatial_resolution_var.get()}')
            self.print_rainbow_dashes()
        ...
        if self.low_noise_range_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -nodata 0 -no_world_file -no_kml -cpu64 -range -cores {self.cores_var.get()} -false -fail -fill 1000 -quiet -keep_class 2 -otif -odir "{low_noise_range_dir}" -step {self.spatial_resolution_var.get()}')
            self.print_rainbow_dashes()
        ...
        if self.class_7_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -cpu64 -cores {self.cores_var.get()} -step {self.spatial_resolution_var.get()} -false -fail -quiet -no_world_file -no_kml -nodata 0 -quiet -keep_class 7 -point_density -otif -odir "{class_7_dir}"')
            self.print_rainbow_dashes()
        ...
        if self.default_density_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -no_world_file -no_kml -cpu64 -cores {self.cores_var.get()} -point_density -false -fail -quiet -keep_class 1 -otif -odir "{default_density_dir}" -step {self.spatial_resolution_var.get()}')
            self.print_rainbow_dashes()
        ...
        if self.ground_density_var.get():
            ...
            os.system(f'lasgrid -i "{self.input_dir_path}\\*.las" -no_world_file -no_kml -cpu64 -cores {self.cores_var.get()} -point_density -false -fail -quiet -keep_class 2 -otif -odir "{ground_density_dir}" -step {self.spatial_resolution_var.get()}')
            self.print_rainbow_dashes()
