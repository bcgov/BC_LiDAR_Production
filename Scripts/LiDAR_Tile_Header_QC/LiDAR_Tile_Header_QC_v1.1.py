...
"""
This program is created to check LiDAR tile header information after they have been produced as a final product.
...
def browse_button():
    ...
    incorrectly_named_files = [file for file in lidar_files if not re.match(r'^(B|b)(C|c)_\d\d\d[A-Za-z]\d\d\d_\d_\d_\d_xyes_(\d|\d\d)_(utm08|UTM08|utm8|UTM8|utm09|UTM09|utm9|UTM9|utm10|UTM10|utm11|UTM11)_\d\d\d\d\d\d\d\d_\d\d\d\d\d\d\d\d.laz$', file)]
...
def find_key_value_pairs(directory, key_value_pairs):
    ...
    pattern = r'^(B|b)(C|c)_\d\d\d[A-Za-z]\d\d\d_\d_\d_\d_xyes_(\d|\d\d)_(utm08|UTM08|utm8|UTM8|utm09|UTM09|utm9|UTM9|utm10|UTM10|utm11|UTM11)_\d\d\d\d\d\d\d\d_\d\d\d\d\d\d\d\d.txt$'
...
root.title("LiDAR Tile Header QC v1.1")
root.iconbitmap(r"Z:\SPENCER_FLOYD\.ico\LiDAR_Strip_Header_QC.ico")
