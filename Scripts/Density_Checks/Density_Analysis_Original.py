...
def run_density_check(input_dir, custom_output_dir=None):
    ...
    if custom_output_dir:
        output_dir = os.path.join(custom_output_dir, "Last_Return_Density_Rasters")
    else:
        output_dir = os.path.join(input_dir, "Last_Return_Density_Rasters")
    ...
    try:
        command = [
            ...
            "-otif",
            "-odir", output_dir,
            "-i", os.path.join(input_dir, "*.laz")
        ]

        print(f"Running lasgrid64 on all .las files in {input_dir}...")
        ...
        log_path = os.path.join(output_dir, "lasgrid64_log.txt")
        with open(log_path, "w") as log_file:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"lasgrid64 failed. See {log_path} for details.")

        print("Replacing nodata values in generated TIFFs...")
        replace_nodata_parallel(glob.glob(os.path.join(output_dir, "*.tif")))
        ...
        messagebox.showerror("Error", f"lasgrid64 failed:\n{e}")
    except Exception as e:
